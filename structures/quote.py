...
class QuoteSeries:
    ...
    def __str__(self):
        ...
        return self.name + " : " + str(
            len(self.data)) + " quotes [" + dateFromStr + " - " + dateToStr + "], last " + str(self.data[-1].c)
    ...
    def loadfromfile(name, filename):
        ...
        for line in f.read().splitlines():
            ...
            date = datetime.strptime(entries[0], '%y-%m-%d') if len(entries[0]) == 10 else datetime.strptime(entries[0],
                                                                                                             '%Y-%m-%d %H:%M:%S')
            quote = Quote(
                date,
                float(entries[1]),
    ...
    def savetofile(self, filename):
        ...
        for q in self.data:
            f.write(
                str(q.date) + "," + str(q.o) + "," + str(q.h) + "," + str(q.l) + "," + str(q.c) + "," + str(q.v) + "\n")
        f.close()
