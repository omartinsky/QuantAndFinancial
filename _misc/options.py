...
class CRR:
    ...
    def calculate(self):
        # check inputs
        if (self.side != CRR.call and self.side != CRR.put): raise ValueError(
            "'side' - Option side must be either call or put")
        if (self.style != CRR.american and self.style != CRR.european): raise ValueError(
            "'style' - Option style must be either american or european")
